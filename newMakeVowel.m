...
% y=jansMakeVowel(dur,sampleRate, F0, f1, f2, f3, f4);
...
%% % Based on the code above - lets calculate the peak harmonic at F0,
% peak harmonic at F1, peak harmonic at F2

% Calculate harmonics at F0
% This is straighforward -
numHarmonics = 10;  % Set the number of harmonics you want to calculate
harmonicsF0 = F0 * (1:numHarmonics);  % First few harmonics of F0

% Now, let's calculate the frequency response of the filter (formants)
freqz(B, A, sampleRate, fs);

% Display harmonics
disp('Harmonics at F0:');
disp(harmonicsF0);

% Analyze how harmonics relate to F1
if f1 > 0
    disp(['Harmonics close to F1 (', num2str(f1), ' Hz):']);
    disp(harmonicsF0(abs(harmonicsF0 - f1) < 100));  % Find harmonics near F1
end